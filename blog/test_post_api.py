...
class PostApiTestCase(TestCase):
  ...
  def setUp(self):
    self.u1 = get_user_model().objects.get_or_create(email="user1@gmail.com", password="pwd_user_1")
    self.u2 = get_user_model().objects.get_or_create(email="user2@gmail.com", password="pwd_user_2")
    self.posts = [
      Post.objects.create(
                author=self.u1,
    ...
    self.post_lookup = {f"{p.id}": p for p in posts}

    # Override test client
    self.client = APIClient()
    token_u1 = Token.objects.create(user=self.u1)
    self.credentials(HTTP_AUTHORIZATION="Token " + token_u1)
